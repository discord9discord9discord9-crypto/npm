...
<head>
    ...
    <script>
        // Image preload loop: load into hidden Image, swap only on success.
        // This prevents flicker when frame is loading or unavailable.
        {% if autoplay %}
        (function() {
            var display = document.getElementById('stream-frame');
            var pending = false;
            function load() {
                if (pending) return;
                pending = true;
...
def frame():
    ...
    if data:
        ...
        # Return a minimal 1x1 white JPEG placeholder (never 404)
        # This keeps the image element valid while waiting for first frame
        ...
        resp = make_response(placeholder_jpeg)
        resp.mimetype = 'image/jpeg'
        resp.headers["Cache-Control"] = "no-store, no-cache, must-revalidate, max-age=0"
        resp.headers["Pragma"] = "no-cache"
        resp.headers["Expires"] = "0"
